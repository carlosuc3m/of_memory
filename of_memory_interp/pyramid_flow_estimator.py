...
class FlowEstimator(nn.Module):
    ...
    flow correction.
    """
    def __init__(self, num_convs: int, num_filters: int, filters: int, sublevel: int, level: int):
        """
        Args:
        ...
        layers = []
        in_filters = 0
        for ll in range(sublevel):
            in_filters += filters * (2 ** ll)
            if ll >= level:
                break
        ...
        layers.append(nn.Conv2d(in_channels=num_filters, out_channels=half_filters, kernel_size=1))
        layers.append(nn.LeakyReLU(0.2, inplace=True))

        # final 1×1 conv → 2 channels, *no* activation
        layers.append(nn.Conv2d(in_channels=half_filters, out_channels=2, kernel_size=1))
