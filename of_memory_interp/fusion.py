    ...
    def __init__(self, config: Options):
        ...
        for i in range(1, -1, -1):
            ...
            level_convs = nn.ModuleList([
                ...
                nn.Conv2d(in_channels=num_filters + tot, out_channels=num_filters, kernel_size=3, padding=1),
                nn.Conv2d(in_channels=num_filters, out_channels=num_filters, kernel_size=3, padding=1),
            ])
            self.convs.append(level_convs)
            in_chans = num_filters
        self.convs = nn.ModuleList(list(reversed(self.convs)))
