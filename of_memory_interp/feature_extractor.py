...
class SubTreeExtractor(nn.Module):
    ...
    def __init__(self, config: Options):
        ...
        for i in range(n):
            out_ch = k << i
            self.convs.append(nn.Conv2d(in_channels=start_chann, out_channels=out_ch, kernel_size=3, padding=1))
            self.convs.append(nn.Conv2d(in_channels=out_ch, out_channels=out_ch, kernel_size=3, padding=1))
            start_chann = out_ch
