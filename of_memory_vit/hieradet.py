...
class Hiera(nn.Module):
    ...
    def __init__(
        ...
        self.model2 = nn.Sequential(
            ...
            nn.Conv2d(128, 128, kernel_size=3, padding=1), nn.LeakyReLU(inplace=True),
            nn.Conv2d(128, 48, kernel_size=1, padding=0),
        )
    ...
    def forward(self, x: torch.Tensor, x2: torch.Tensor) -> List[torch.Tensor]:
        ...
        x = x + self.model(x2).permute(0, 2, 3, 1)

        outputs = []
        for i, blk in enumerate(self.blocks):
            x = blk(x)
            if i == 1:
                x = x + self.model2(x2).permute(0, 2, 3, 1)
